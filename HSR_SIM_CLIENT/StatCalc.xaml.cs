...

{
    ...
    private void LoadStatTable(Dictionary<string, string> table)
    {
        ...
        {
            SelectedStats.Add(new SelectedItem(item.Key));
        }
    ...
    private void RefreshCb()
    {
        ...
        {
            Scenarios.Add(new SelectedItem(Path.GetFileName(t)) { IsSelected = false });
        }
        ...
        {
            SelectedItem? selItem = Scenarios.FirstOrDefault(x => x.Name.Equals(item));
            if (selItem != null)
        ...
        foreach (var t in files)
        {
            Profiles.Add(new SelectedItem(Path.GetFileName(t)) { IsSelected = false });
        }
        ...
        foreach (var item in IniF.IniReadValue(GetType().Name, nameof(Profiles)).Split(Delimiter)
                     .Where(x => !string.IsNullOrEmpty(x)))
        {
            SelectedItem? selItem = Profiles.FirstOrDefault(x => x.Name.Equals(item));
            if (selItem != null)
                selItem.IsSelected = true;
        }
