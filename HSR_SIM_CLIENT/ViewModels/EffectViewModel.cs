...
public class EffectViewModel (Effect? effect,Formula? formula=null)
{
    ...
    public string? Value =>  (effect is EffWeaknessImpair ew)?ew.Element.ToString() : effect?.DynamicValue??false?formula?.DescendantsAndSelfEffects().FirstOrDefault(x=>x.TraceEffect==effect)?.TraceTotalValue.ToString(CultureInfo.InvariantCulture)??effect?.Value.ToString() :effect?.Value.ToString();
