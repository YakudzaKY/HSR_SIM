...
public class UnitViewModel(Unit unit)
{
    ...
    public List<BuffViewModel> PassiveBuffs => unit.PassiveBuffs
        .Where(x => x.Truly(x, null, null, null) && !x.IsTargetCheck).Select(buff => new BuffViewModel(buff)).ToList();

    public List<BuffViewModel> InactivePassiveBuffs => unit.PassiveBuffs
        .Where(x => !x.Truly(x, null, null, null) && !x.IsTargetCheck).Select(buff => new BuffViewModel(buff)).ToList();

    public List<BuffViewModel> TargetCheckBuffs => unit.PassiveBuffs.Where(x => x.IsTargetCheck)
        .Select(buff => new BuffViewModel(buff)).ToList();
    ...
    public List<WeaknessRec> Weakness
    {
        get
        {
            ...
            foreach (var weakness in unit.NativeWeaknesses) res.Add(new WeaknessRec(weakness.ToString(), []));
            //weakness impair
            foreach (var weakness in unit.GetBuffEffectsByType(typeof(EffWeaknessImpair), ent: null,
                         excludeCondition: null))
                res.Add(new WeaknessRec(
                    ((EffWeaknessImpair)weakness.Value.First(x => x is EffWeaknessImpair)).Element.ToString(),
                    new BuffViewModel[] { new(weakness.Key) }));
    ...
    public List<UnitStatRec> Resists
    {
        ...
        {
            var res = new List<UnitStatRec>();
            foreach (var elem in ((Ability.ElementEnm[])Enum.GetValues(typeof(Ability.ElementEnm))).Where(x =>
                         x != Ability.ElementEnm.None))
            {
                var resVal = unit.Resists(ent: null, elem: elem);
                if (resVal.Result != 0 || resVal.TraceBuffs().Any()) res.Add(new UnitStatRec(elem.ToString(), resVal));
            }
    ...
    public List<UnitStatRec> DebuffResists
    {
        get
        {
            ...
            foreach (var debuffRes in unit.NativeDebuffResists)
            {
                ...
                if (resVal.Result != 0) res.Add(new UnitStatRec(debuffRes.Debuff.Name, resVal));
            }

            //all debuff res 
            var allDebuffRes = unit.DebuffResists(ent: null, effect: null);
            if (allDebuffRes.Result != 0) res.Add(new UnitStatRec("All debuff res", allDebuffRes));
            return res;
        }
    }
    ...
    public List<UnitStatRec> AbilityTypeBoosters
    {
        ...
        {
            var res = new List<UnitStatRec>();
            foreach (var aType in (Ability.AbilityTypeEnm[])Enum.GetValues(typeof(Ability.AbilityTypeEnm)))
            {
                var resVal = unit.AbilityTypeMultiplier(ent: null, abilityType: aType);
                if (resVal.Result != 0) res.Add(new UnitStatRec(aType.ToString(), resVal));
            }
    ...
    public record UnitStatRec(string Name, Formula StatFormula)
    {
        ...
        public IEnumerable<BuffViewModel> Buffs =>
            StatFormula.TraceBuffs().Select(buff => new BuffViewModel(buff, StatFormula));
    }
