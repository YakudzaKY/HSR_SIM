        ...
        if (!TriggersHandled)
        {
            ...
            switch (this.ParentStep.ActorAbility.Element)
            {
                case ElementEnm.Physical:
                    this.TryDebuff(new BuffBleedWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff), 1.5);
                    break;
                case ElementEnm.Fire:
                    this.TryDebuff(new BuffBurnWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff), 1.5);
                    break;
                case ElementEnm.Ice:
                    this.TryDebuff(new BuffFreezeWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff), 1.5);
                    break;
                case ElementEnm.Lightning:
                    this.TryDebuff(new BuffShockWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff), 1.5);
                    break;
                case ElementEnm.Wind:
                    this.TryDebuff(
                        new BuffWindShearWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff)
                        { Stack = this.TargetUnit.Fighter.IsEliteUnit ? 3 : 1 }, 1.5);
                    break;
                case ElementEnm.Quantum:
                    this.TryDebuff(
                        new BuffEntanglementWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff, this),
                        1.5);

                    break;
                case ElementEnm.Imaginary:
                    this.TryDebuff(
                        new BuffImprisonmentWB(this.SourceUnit, this.SourceUnit.Fighter.WeaknessBreakDebuff, this),
                        1.5);
                    break;
                default:
