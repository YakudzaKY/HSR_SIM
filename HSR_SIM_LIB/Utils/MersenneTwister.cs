...
public class MersenneTwister
{
    ...
    [ThreadStatic] private static MersenneTwister _shared;
    ...
    /// </summary>
    public static MersenneTwister Shared => _shared ??= new MersenneTwister(GetRandomSeed());
    ...
    private void InitGenerator(uint seed)
    {
        ...
        var values = MemoryMarshal.Cast<Vector128<uint>, uint>(_state);
        values[0] = seed;
        for (var i = 1; i < Length32; i++)
            values[i] = (uint)(1812433253ul * (values[i - 1] ^ (values[i - 1] >> 30)) + (uint)i);
        _index = Length32;

        var parity = Vector128.Create(0x00000001u, 0x00000000u, 0x00000000u, 0x13c9e684u);
        var v = Sse2.And(_state[0], parity);
    ...
    {
        var z = Sse2.ShiftRightLogical128BitLane(c, 1);
        z = Sse2.Xor(z, a);
        var v = Sse2.ShiftLeftLogical(d, 18);
        z = Sse2.Xor(z, v);
        var x = Sse2.ShiftLeftLogical128BitLane(a, 1);
        z = Sse2.Xor(z, x);
        var y = Sse2.ShiftRightLogical(b, 11);
        var mask = Vector128.Create(0xdfffffefu, 0xddfecb7fu, 0xbffaffffu, 0xbffffff6u);
        y = Sse2.And(y, mask);
        return Sse2.Xor(z, y);
    }
    ...
    public ulong NextUInt64()
    {
        ...
        var values = MemoryMarshal.Cast<Vector128<uint>, ulong>(_state);
        var r = values[_index >> 1];
        _index += 2;
        return r;
    }
    ...
    public int Next(int minValue, int maxValue)
    {
        ...
        if (maxValue <= minValue)
            throw new ArgumentOutOfRangeException(nameof(maxValue),
                $"Значение должно быть больше, чем {nameof(minValue)}.");
